...
@section('content')
        ...
        <div class="d-flex justify-content-center mb-5">
            <div style="max-width: 700px">
                <p class="text-paragraph text-center">
                    We are committed to supplying excellent and high-quality materials for customers needs at all times.
                    We define a 'high-quality' product as one that is reliable in its quality, not just that it is safe and comprised of high-quality materials.
        ...
        <div class="row">
            @foreach ($listProds as $item)
                @component('components.card.card-product')
                    @slot('link')
                        {{route('productDetail', ['slug' =>  $item->image]) }}
                    @endslot
                    @slot('title')
                        {{$item->title}}
                    @endslot    
                    @slot('img')
                        {{asset('image/product/list/'.$item->image.".png".versioning())}}
                    @endslot
                @endcomponent    
    ...
    <div class="mt-5 belt__asli">
        <div class="container">
            <h3 class="mx-lg-0 mx-2 mb-0">This website is the only official website of Trustia Asia Corporation Pte Ltd</h3>
